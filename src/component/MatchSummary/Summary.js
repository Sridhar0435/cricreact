    ...
    return (
      <section>
        <article className="articleTable hoverable animated bounceInLeft delay-1s">
          {inSummary !== "" ? (
            <PartnerShip
              sId={this.props.seriesIdSummary}
              mId={this.props.matchId}
              firstIn={
                showInSummary.innings.length > 1
                  ? showInSummary.innings[0].shortName
                  : ""
              }
              secondIn={
                showInSummary.innings.length > 1
                  ? showInSummary.innings[1].shortName
                  : ""
              }
            />
          ) : (
        ...
        {typeof showInSummary !== "undefined" &&
        showInSummary.matchSummary.status !== "UPCOMING" ? (
          <article className="articleTable hoverable animated bounceInLeft delay-1s">
            <h5>
              <span className="PartnershipTitle">Teams</span>
            </h5>
            <table className="responsive-table highlight">
              ...
              <tbody>
                {summaryPageShow !== "" &&
                summaryPageShow.metaData.state !== "Upcoming" ? (
                  summaryPageShow.teams.map((item, index) => (
                    <React.Fragment key={index}>
                      {group !== item.groupName &&
                      item.groupName.includes("Group") ? (
                        <tr className="">
                          <td colSpan="10" className="tdfull">
                            {(group = item.groupName)}
                          </td>
                        </tr>
                      ) : (
                  ...
                  <tr>
                    <td colSpan="10" className="upcomingmatchsummary">
                      Upcoming Match...
                    </td>
                  </tr>
                )}
            ...
            <div>
              <ul className="statusSumm">
                <li>
                  {summaryPageShow !== ""
                    ? summaryPageShow.metaData.status
                    : ""}
                </li>
                <li>
                  {summaryPageShow !== ""
                    ? summaryPageShow.metaData.series
                    : ""}
                </li>
              </ul>
            </div>
        ...
        <article className="articleTable hoverable animated bounceInLeft delay-1s">
          <header className="seriesName">
            <span>
              {summaryPageShow !== "" ? summaryPageShow.metaData.series : ""}
            </span>
          </header>
          <div className="row">
            {seriesTeamNames !== ""
              ? seriesTeamNames.seriesTeams.teams.map((item, index) => (
